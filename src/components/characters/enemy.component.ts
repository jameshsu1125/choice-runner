...
import { enemyEntityConfig, enemyEntityPresetConfig } from "../../configs/presets/enemy.preset";
...
export class EnemyComponent extends Container {
    ...
    private createEnemy(config: (typeof enemyEntityConfig)[number]["data"], time: number): void {
    ...
    public removeStateByName(name: string): void {
        const [state] = this.enemyState.filter((state) => state.target.enemyName === name);

        if (state) {
            state.target.destroy();
        }

        this.enemyState = this.enemyState.filter((state) => state.target.enemyName !== name);
    }

    public loseBlood(enemy: Sprite) {
        const [state] = this.enemyState.filter((state) => state.target.enemyName === enemy.name);
    ...
    public adjustSpeedChange(currentTime: number, oldDuration: number, newDuration: number): void {
        ...
        this.enemyState.forEach((state) => {
            ...
            const currentPercent = (currentTime - state.startTime + timeOffset) / oldDuration;
