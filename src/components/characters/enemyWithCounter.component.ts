...
export default class EnemyWithCounterComponent extends Container {
    ...
    constructor(
        ...
        this.healthBarFill.setDepth(999);
        ...
        colorGraphics.generateTexture(
            `${this.config.blood.color}`,
            width,
            height
        );
    ...
    private addCollision(enemy: Phaser.Physics.Arcade.Sprite) {
        ...
        if (layoutContainers.firepower) {
            layoutContainers.firepower?.firepowerContainer.forEach(
                (firepower) => {
    ...
    public setPositionByPercentage(percentage: number) {
        ...
        const y =
            (this.scene.scale.height + enemy.displayHeight) * currentPercent;
