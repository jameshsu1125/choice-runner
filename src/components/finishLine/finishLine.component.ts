import { BitmapMask, Container, Graphics, Image, Scene } from "../../configs/constants/constants";
...
export class FinishComponent extends Container {
    ...
    private createLine(): void {
        ...
        this.finishLine.setPosition(
            0,
            getAlign(this.finishLine, this.scene, "TOP") - this.finishLine.displayHeight
        );
    ...
    public adjustSpeedChange(currentTime: number, oldDuration: number, newDuration: number): void {
        ...
        const currentPercent = Math.abs((currentTime - this.startTime + timeOffset) / oldDuration);
