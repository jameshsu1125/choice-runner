...
export class FirepowerComponent extends Container {
    ...
    private bulletPositionTracker = new Map<string, { y: number; frameCount: number }>();
    ...
    public fire(delta: number): void {
        ...
        this.player.players.forEach((player, playerIndex) => {
            ...
            this.scene.time.delayedCall(delayMs, () => {
                ...
                const currentRatio =
                    this.level === 1 ? ratio.level1 : ratio.level2;
                ...
                const finalVelocityY = Math.max(bulletVelocityY, FIREPOWER_CONSTANTS.MAX_VELOCITY_Y);
    ...
    private _isBulletStuck(firepower: Sprite): boolean {
        ...
        if (velocityY === 0 || Math.abs(velocityY) < FIREPOWER_CONSTANTS.NEAR_ZERO_VELOCITY_THRESHOLD) {
            return true;
        }
