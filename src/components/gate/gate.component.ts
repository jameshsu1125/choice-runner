...
import { gateEntityConfig, gateEntityPresetConfig } from "../../configs/presets/gate.preset";
...
export class GateComponent extends Container {
    ...
    private createGate(config: { quadrant: TQuadrant; count: number }, time: number): void {
    ...
    public removeStateByName(name: string): void {
        const [state] = this.gateState.filter((state) => state.target.gateName === name);

        this.gateState = this.gateState.filter((state) => state.target.gateName !== name);
