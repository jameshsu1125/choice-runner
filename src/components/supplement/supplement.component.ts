...
export class SupplementComponent extends Container {
    ...
    private increaseSupplementCountByType: (type: "ARMY" | "GUN", supplementName: string) => void;

    constructor(
        scene: Phaser.Scene,
        increaseSupplementCountByType: (type: "ARMY" | "GUN", supplementName: string) => void
    ) {
        super(scene, 0, 0);
    ...
    public fire(time: number, config: (typeof supplementEntityConfig)[number]): void {
